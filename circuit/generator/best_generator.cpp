...
int main() {
    ...
    while (node_count < OPTIONS::bin_o + INPUT_AMOUNT + endsize) {
        ...
        if (uniform(rng) <= OPTIONS::uni_t) {
            ...
            int in1 = 0;
            while (in1 == 0)
                in1 = std::abs(bin2(rng) - (int)unused.size());
            ...
            while (link1 == 0)
                link1 = std::abs(bin3(rng) - node_count);
        }
        int link2 = 0;
        while (link2 == link1 || link2 == 0)
            link2 = std::abs(bin3(rng) - node_count);
        unused.erase(link2);
    ...
    while (unused.empty() || unused.size() % 10 != 0) {
        ...
        int link1 = 0, link2 = 0;
        while (link1 == 0 || unused.find(link1) != unused.end())
            link1 = std::abs(bin(rng) - node_count);
        while (link2 == 0 || link2 == link1 || unused.find(link1) != unused.end())
            link2 = std::abs(bin(rng) - node_count);
        conn.push_back({link1, link2});
        unused.insert(++node_count);
    }
    std::vector<int> off;

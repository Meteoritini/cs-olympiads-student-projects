...
struct Node {
    ...
    // Default constructor
    Node(int32_t _type, int32_t _id, int32_t _link_a, int32_t _link_b) : type(_type), id(_id), link_a(_link_a), link_b(_link_b) {}
};
...
void toposort_nodes() {
    ...
    for (size_t i = 1; i <= INPUT_NODES; i++)
...
int main(int argc, char const* argv[]) {
    ...
    if (argc != 3)
        program_abort(EXIT_WRONG_USAGE);
    ...
    for (int32_t i = 0; i < node_count - INPUT_NODES; ++i) {
        ...
        if (node_value[0] > 16 || node_value[0] < 0 || node_value[1] <= (int32_t)INPUT_NODES) {
            program_abort(EXIT_INVALID_NODE_DATA);
        }
    ...
    std::sort(input_nodes.begin(), input_nodes.end(), [&](const Node& a, const Node& b) { return a.id < b.id; });
